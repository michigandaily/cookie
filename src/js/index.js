...
window.onload = async () => {
	...
	const { entries } = await import('../../sink.config.js');
	...
	const setEntry = (e, parameters = {}) => {
		...
		['hed', 'dek', 'note', 'source', 'byline'].forEach((property) => {
			$(`#${property}-checkbox`).disabled = !Object.hasOwn(entries[e], property);
		});
	...
	const getDisplayOptionsFromQueryParams = () => {
		return Object.fromEntries(
			displayOptions.filter((d) => params.has(d.name)).map((d) => [d.name, params.get(d.name)])
		);
	};
	...
	displayOptions.forEach((option) => {
		...
		option.addEventListener('change', ({ target: { name, checked } }) => {
			if ((name === 'home' && !checked) || (name !== 'home' && checked)) {
				params.delete(name);
			} else if ((name === 'home' && checked) || (name !== 'home' && !checked)) {
